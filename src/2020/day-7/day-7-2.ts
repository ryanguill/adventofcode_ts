...
/*
  ...
  execute solution:
    clear && npm run compile && time node --trace-warnings --async-stack-traces dist/2020/day-7/day-7-2.js

  unit test
    clear && npm run compile && ./node_modules/.bin/mocha --ui tdd dist/2020/day-7/day-7-2.js
...
if (!isRunningUnitTests()) {
  (async function main() {
    ...
    const answer = count(input, 'shiny gold')
...
function count(
  ...
  return elements.reduce((acc, [n, element]) => {
    return acc + n + (n * count(data, element));
  }, 0);
}
